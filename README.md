...
## Modifications
This package has been modified to suit the needs of the AG arc. The key modifications are:
- Introduction of a new point type to match the data fields generated by the sick multiscan 136. This replaces the default PointXYZINormal and various other PCL point types used in the default fast lio
- Addition of a new sensor type and functions to process the sick data fields
- Creation of a second pointcloud storage variable to store and save the groundless point cloud
- Creation of a function to publish the deskewed pointcloud to have the ground removed from it
- Creation of a buffer to store the state variables along with their timestamps, such that groundless pointclouds can be added to the map
- Creation of a subscriber to subscribe to the groundless pointclouds, re-asscociate them with their states, transform them into the global frame, and add them to the groundless map variable. 
## Overview
