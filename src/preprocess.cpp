...
//         pl_full[i].curvature =
//             msg->points[i].offset_time / float(1000000);  // use curvature as time of each laser
//             points

//         bool is_new = false;
//         if ((abs(pl_full[i].x - pl_full[i - 1].x) > 1e-7) || (abs(pl_full[i].y - pl_full[i -
//         1].y) > 1e-7) ||
...
//               || (abs(pl_full[i].z - pl_full[i-1].z) > 1e-7))
//               && (pl_full[i].x * pl_full[i].x + pl_full[i].y * pl_full[i].y + pl_full[i].z *
//               pl_full[i].z > (blind * blind)))
//           {
...
void Preprocess::sick_handler(const sensor_msgs::msg::PointCloud2::UniquePtr& msg) {
  ...
  if (feature_enabled) {
    ...
    for (uint i = 0; i < plsize; i++) {
      if (pl_orig.points[i].t == 0U) continue;
    ...
    for (int i = 0; i < pl_orig.points.size(); i++) {
      ...
      // Some points can have missing per-point time. Keeping them can degrade deskew quality.
      if (pl_orig.points[i].t == 0U) continue;
...
int Preprocess::plane_judge(const PointCloudXYZI& pl, vector<orgtype>& types, uint i_cur,
...
bool Preprocess::edge_jump_judge(const PointCloudXYZI& pl, vector<orgtype>& types, uint i,
                                 Surround nor_dir) {
  if (nor_dir == 0) {
